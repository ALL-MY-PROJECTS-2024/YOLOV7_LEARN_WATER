...
def stream_video():
    ...
    source_id = str(uuid4())  # 고유 ID 생성
    stop_event = Event()

    with resource_lock:  # 동시성 보호
    ...
    def stream_worker():
        ...
        try:
            ...
            # 스트림 종료 시 호출
            print(f"Stream worker for {source_id} exited.")
        finally:
            cleanup_stream_resources(source, source_id)

    return Response(stream_worker(), mimetype='multipart/x-mixed-replace; boundary=frame')



def cleanup_stream_resources(source, source_id):
    """스트림 리소스 정리"""
    with resource_lock:  # 동시성 보호
        print(f"Cleaning up resources for stream {source_id}...")
        if source in stream_threads and source_id in stream_threads[source]:
            del stream_threads[source][source_id]
            if not stream_threads[source]:  # 요청이 없는 경우
                del stream_threads[source]

        if source_id in stream_events:
            del stream_events[source_id]

    # OpenCV 리소스 해제
    cv2.destroyAllWindows()
    print(f"Resources for stream {source_id} have been cleaned up.")
...
def stop_stream():
    ...
    source_id = data.get('streamId')

    if not source_id or source_id not in stream_events:
        return jsonify({"error": "Invalid or missing 'streamId' parameter"}), 400

    stop_event = stream_events[source_id]
    stop_event.set()  # 스트림 종료 신호 전달

    # 리소스 정리를 위해 cleanup 호출
    cleanup_stream_resources(None, source_id)

    return jsonify({"message": f"Stream with ID {source_id} stopped."}), 200
