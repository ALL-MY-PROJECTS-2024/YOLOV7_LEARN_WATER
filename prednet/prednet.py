...
class PredNet(Layer):
   ...
   def build(self, input_shape):
       # self.input_spec = [InputSpec(shape=input_shape)]
        ...
        # ConvLSTM 로직을 대체하거나 호출되는 레이어를 설정
        for l in range(self.nb_layers):
            for c in ['i', 'f', 'c', 'o']:
                act = self.LSTM_activation if c == 'c' else self.LSTM_inner_activation
                self.conv_layers[c].append(
                    Conv2D(self.R_stack_sizes[l], self.R_filt_sizes[l], 
                        padding='same', activation=act, data_format=self.data_format)
                )
...
#
    ...
    def step(self, a, states):
        ...
        r_tm1 = states[:self.nb_layers]  # 이전 R 상태
        c_tm1 = states[self.nb_layers:2*self.nb_layers]  # 이전 C 상태
        e_tm1 = states[2*self.nb_layers:3*self.nb_layers]  # 이전 Error 상태
